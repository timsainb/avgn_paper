...
def log_mel_spectrogram(spectrogram, fs, hparams):
    """Compute the log mel spectrogram from a spectrogram."""
    melSpectrogram = melspectrogram(spectrogram, fs, stft_system, hparams)
    return log_spectrogram(melspectrogram, hparams)


def create_stft_system(hparams, fs):
    # creates object for performing spectrogramming/reconstruction
    hop_size = int(hparams.hop_length_ms / 1000 * fs)
    stft_system = GaussTF(hop_size=hop_size, stft_channels=hparams.n_fft)
    return stft_system


def melspectrogram(y, fs, stft_system, hparams):
    ...
    log_spec = log_spectrogram(spectrogram, hparams)
    ...
    return np.dot(mel_basis, log_spec), mel_basis


def pseudo_unmel_spectrogram(mel_spectrogram, fs, hparams):
...
class GaussTF(object):
    ...
    def __init__(self, hop_size=256, stft_channels=1024):
        assert (
            np.mod(stft_channels, 2) == 0
    ...
    def idgt(self, X, hop_size=None, stft_channels=None):
        ...
        return idgtreal(X.astype(np.complex128), g_synthesis, hop_size, stft_channels)[
            0
        ]
    ...
    def invert_spectrogram(self, spectrogram, stft_channels=None, hop_size=None):
        ...
        tgrad, fgrad = modgabphasegrad(
            "abs", spectrogram, g_analysis, hop_size, stft_channels
        )
    ...
    def spectrogram(self, time_signal, normalize=True, **kwargs):
...
class GaussTruncTF(GaussTF):
    ...
    def __init__(self, hop_size=256, stft_channels=1024, min_height=1e-4):
        super().__init__(hop_size, stft_channels)
        self.min_height = min_height

    def _analysis_window(self, x):
        Lgtrue = np.sqrt(
            -4 * self.hop_size * self.stft_channels * np.log(self.min_height) / np.pi
        )
        LgLong = np.ceil(Lgtrue / self.stft_channels) * self.stft_channels

        x = (1 / Lgtrue) * np.concatenate(
            [np.arange(0.5 * LgLong), np.arange(-0.5 * LgLong, 0)]
        )
    ...
    def _synthesis_window(self, X, hop_size, stft_channels):
        ...
        return gabdual(
            ...
            hop_size,
            stft_channels,
            8 * max(stft_channels, self.stft_channels),
        )
